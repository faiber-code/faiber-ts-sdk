...
test("the facade and AI catalog cover every sandbox service exactly once", () => {
  const expected = ["idp", "profile", "modules", "asset", "payment", "messenger", "crm", "lms", "reservation", "session", "version", "flow", "knowledge", "chat", "social", "state", "drm"];
...
test("all 17 public service connections share auth, transport, and facade configuration", async () => {
  const requests = [];
...
test("every published workspace package includes developer documentation", async () => {
  ...
  for (const name of packageNames) {
    ...
    assert.equal(packageJson.version, "0.5.1", `${packageJson.name} release version`);
    for (const [dependency, version] of Object.entries(packageJson.dependencies ?? {})) {
      if (dependency.startsWith("@faiber/")) assert.equal(version, "0.5.1", `${packageJson.name} -> ${dependency}`);
    }
