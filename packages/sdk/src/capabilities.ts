...
export const FAIBER_SERVICE_CAPABILITIES = [
  ...
  { service: "social", title: "Social", description: "Feeds, posts, comments, reactions, bookmarks, reports, analytics, and moderation.", aiGuidance: "Use self-service routes for a user's content and moderation routes only with explicit permissions.", transports: ["rest", "multipart"], operationCount: 45, environmentVariable: "FAIBER_SOCIAL_URL", auth: "IDP session with social write or moderation permissions." },
  { service: "state", title: "Realtime state", description: "Health, worlds, rooms, authoritative state, WebSocket/UDP replication, prediction, and WASM simulation.", aiGuidance: "Create/join through REST, then use the returned short-lived ticket with StateRealtimeClient.", transports: ["rest", "websocket", "udp"], operationCount: 15, environmentVariable: "FAIBER_STATE_URL", auth: "IDP session for REST; signed join ticket for realtime transports." },
  ...
  { service: "task", title: "Tasks", description: "Projects, work items, sprints, campaigns, approvals, workload and delivery reports.", aiGuidance: "Use Task as the sole generic work authority; preserve project scope, idempotency keys and optimistic versions.", transports: ["rest"], operationCount: 20, environmentVariable: "FAIBER_TASK_URL", auth: "Verified IDP identity and global Task permission plus Task-owned scoped grants." },
