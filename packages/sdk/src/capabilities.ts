...
export const FAIBER_SERVICE_CAPABILITIES = [
  { service: "idp", title: "Identity", description: "Authentication, sessions, users, roles, permissions, OAuth, OIDC, and access control.", aiGuidance: "Authenticate and manage identity here; never store credentials in generated project code.", transports: ["rest"], operationCount: 95, environmentVariable: "FAIBER_IDP_URL", auth: "Public bootstrap plus Bearer/cookie and permission-guarded administration." },
  ...
  { service: "lms", title: "Learning", description: "Courses, classroom-session scheduling and types, room provisioning state, enrollment, assessments, certificates, homework, and reports.", aiGuidance: "Distinguish learner academy routes from management routes, preserve UUID identifiers, and honor classroom-session lock state before opening provisioned Session rooms.", transports: ["rest"], operationCount: 122, environmentVariable: "FAIBER_LMS_URL", auth: "IDP session with learner context or LMS management permissions." },
  ...
  { service: "session", title: "Live sessions", description: "Live rooms, access tokens, playback, recordings, and analytics.", aiGuidance: "Request short-lived room access from the service; never synthesize media access tokens.", transports: ["rest"], operationCount: 29, environmentVariable: "FAIBER_SESSION_URL", auth: "IDP session with room permissions for management operations." },
