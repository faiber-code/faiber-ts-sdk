...
export class InferaClient {
  ...
  constructor(readonly service: ServiceName, readonly config: FaiberSdkConfig) {
    ...
    if (config.refreshAuth) this.axios.interceptors.response.use(undefined, async (error: AxiosError) => {
      const request = error.config as (AxiosRequestConfig & { [RETRY]?: boolean }) | undefined;
      const shouldRefresh = config.shouldRefreshAuth?.(error) ?? error.response?.status === 401;
      if (!request || request[RETRY] || !shouldRefresh) throw error;
      request[RETRY] = true;
      try {
        ...
        return this.axios.request(request);
      } catch (refreshError) { await config.tokenProvider?.setTokens(null); await config.onAuthFailure?.(refreshError); throw refreshError; }
    });
  }
